...
Write a function named findHappiness that takes in an array of strings
...
const findHappiness = (arr) => {
  // Solution code here...
  let str = ['this is great :)', 'wow', 'whyyyyyy :(', ':)))))']
  let newArr = arr.filter(ele => ele.includes(':)'))
  return newArr
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  let regex = /[0-9]/gi
  let newArr = arr.map(ele => ele.match(regex).join(""))
  return newArr
};
...
const allHappy = (arr) => {
  // Solution code here...
  let newArr = arr.every(ele => ele.includes(":)"))
  return  newArr
};
