...
Write a function named count that, given an integer and an array of arrays, uses either filter, map, or reduce to count the amount of times
 the integer is present in the array of arrays.
...
const count = (target, input) => {
  ...
  let newArr =[]
let counter = 0
input.forEach(ele => newArr.push(...ele))
 newArr.filter(ele => { if(ele === target ) counter++})
 return counter
};
...
const totalSum = (input) => {
  ...
  let newArr =[]
  input.forEach(ele => newArr.push(...ele))
 let total = newArr.reduce((pre , cur) => pre + cur , 0)
  return total
};
...
function divisibleByFiveTwoToThePower(arr) {
  // Solution code here...
  return arr.map(ele => {
    const filteredArr = ele.filter(num => typeof num === 'number' && num % 5 === 0);
...
let filterData = data.filter((ele , i) => {
  if(ele.gender === 'male' || ele.gender === 'female'){
    i === data.length - 1 ? str += `${ele.name}` : str += `${ele.name} and `
  }
})
return str
};
...
let findShortest = (data) => {
  ...
  let filterData = []
  let length1 = []
  data.forEach(ele => filterData.push(ele.name))
  data.forEach(ele => length1.push(ele.name.length))

return filterData[length1.indexOf(Math.min(...length1))]
};
...
describe('Testing challenge 6', () => {
  test('It should return the name of the shortest character', () => {
    expect(findShortest(starWarsData)).toStrictEqual('C-3PO');
  });
});
