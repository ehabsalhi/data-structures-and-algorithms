...
const raisedToTheThird = (arr) => {
  // Solution code here...
  let newArr = arr.map(ele => Math.pow(ele , 3))
  return newArr
};
...
const forLoopTwoToThe = (arr) => {
  ...
  let newArr =[]
  for( let ele in arr){
    newArr.push(Math.pow( 2 , arr[ele]))
  } 
  return newArr
};
...
const forEachTwoToThe = (arr) => {
  // Solution code here...
  let newArr =[]
  arr.forEach(ele => newArr.push( Math.pow(2 , ele)))
...
const mapTwoToThe = (arr) => {
  // Solution code here...
  let newArr = arr.map(ele => ( Math.pow(2 , ele)))
...
Write a function named extractAbilities that,
 given the array of abilities, uses map to create an array containing only the ability name.
