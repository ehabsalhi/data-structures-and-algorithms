...
Write a function named screenForNames that takes in an array of strings and 
uses Regex to create a new array of only those strings that match the following rules:
...
const screenForNames = (arr) => {
  // Solution code here...
  let regex = /^(Mr\.\s|Mrs\.\s|Ms\.\s|Dr\.\s)([a-zA-z]|[a-zA-z]\s[a-zA-z])/g
  let newArr = arr.filter(ele => ele.match(regex))
  return newArr
}
...
Write a function named toTitleCase that takes in an array of strings and
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let newArr =[]
   arr.forEach(ele => { if(ele.mass > 77) newArr.push(ele.name)  })
  return newArr.join(" - ")
};
...
const sortBy = (property, arr) => {
  // Solution code here...
  if(property === 'name' ){
    let newArr =arr.sort((a,b) => a.name.localeCompare(b.name))
    return newArr
  }else{
...
Write a function named detectTicTacToeWin that accepts a two-dimensional array of strings.
 ...
 Your function should check to see if any row, column, or either diagonal direction has three matching "X" or "O" symbols (non-empty strings), 
 three-in-a-line.
...
columns and diagonals consider writing one helper function that accepts three coordinate pairs and checks the values of the array at those locations.
